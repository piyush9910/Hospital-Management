...
def patientadm():
    ...
    if choicee==1:
        ...
        df = pd.read_csv('Rooms.csv', skiprows=1, names=['S.no', 'Name', 'Price'])
        r = pd.pivot_table(df, index=["Name"])
        print(r)
        with open('Rooms.csv', 'r')as fileZ:
            read = csv.reader(fileZ)
            members = int(input("Choice : "))
            for i in read:
                if i[0] == members:
                    p = i[2] * ndays
                    list2.append(p)

        print("Avaialble Doctors\n")
        df = pd.read_csv('doctors.csv', skiprows=1, names=['S.no', 'Name','Specialist', 'Price'])
        r = pd.pivot_table(df, index=["Name"])
        print(r)
        with open('doctors.csv', 'r')as fileZ:
            read = csv.reader(fileZ)
            members = int(input("Choice : "))
            for i in read:
                if i[0] == members:
